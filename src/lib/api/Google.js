...
const { PUBLIC_URL, G_CLIENT_ID, G_CLIENT_SECRET, G_GEOCODE_KEY } = process.env;
const auth = new googleapis_1.google.auth.OAuth2({
    clientId: G_CLIENT_ID,
    clientSecret: G_CLIENT_SECRET,
    redirectUri: `${PUBLIC_URL}/login`,
});
const maps = maps_1.createClient({ key: G_GEOCODE_KEY, Promise });
...
exports.Google = {
    ...
    geocode: (address) => __awaiter(void 0, void 0, void 0, function* () {
        const { status, json: { results }, } = yield maps.geocode({ address }).asPromise();
        if (status < 200 || status > 299) {
            throw Error("failed to geocode address");
        }
        return parseAddress(results[0].address_components);
    }),
};
